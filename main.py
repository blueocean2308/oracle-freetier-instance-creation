...
print("=== Starting OCI Free Tier Instance Creation Script ===")
print(f"Shape: {OCI_COMPUTE_SHAPE}")
print(f"Display Name: {DISPLAY_NAME}")
print(f"Wait time between retries: {WAIT_TIME}s")
print(f"Subnet ID set: {bool(OCI_SUBNET_ID)}")
print(f"Image ID set: {bool(OCI_IMAGE_ID)}")
...
try:
    ...
    env_has_spaces = any(isinstance(confg_var, str) and " " in confg_var
                        for confg_var in [OCI_CONFIG, OCT_FREE_AD, str(WAIT_TIME),
                                SSH_AUTHORIZED_KEYS_FILE, str(OCI_IMAGE_ID), 
                                OCI_COMPUTE_SHAPE, str(SECOND_MICRO_INSTANCE), 
                                str(OCI_SUBNET_ID), OS_VERSION, str(NOTIFY_EMAIL), EMAIL,
...
logging_step5.addHandler(console)
...
def generate_html_body(instance):
    try:
        ...
        return f"Instance created: {instance.id} - {instance.display_name}"
...
def handle_errors(command, data, log):
    ...
    if (
        ...
        print(f"[RETRY] {data.get('code')} - {message} | Waiting {WAIT_TIME}s...")
...
def read_or_generate_ssh_public_key(public_key_file: Union[str, Path]):
    ...
    if not public_key_path.is_file():
        print("SSH key doesn't exist... Generating SSH Key Pair")
...
def launch_instance():
    ...
    print(f"ADs to try: {oci_ad_name}")
    logging.info("OCI_AD_NAME: %s", oci_ad_name)

    print("Step 3: Getting Subnet...")
    ...
    print(f"Subnet ID: {oci_subnet_id}")
    logging.info("OCI_SUBNET_ID: %s", oci_subnet_id)

    print("Step 4: Getting Image...")
    if not OCI_IMAGE_ID:
        ...
        print(f"Image ID (auto): {oci_image_id}")
        logging.info("OCI_IMAGE_ID: %s", oci_image_id)
    else:
        oci_image_id = OCI_IMAGE_ID
        print(f"Image ID (from env): {oci_image_id}")
    ...
    print("Step 5: Checking if instance already exists...")
    ...
    while not instance_exist_flag:
        ...
        try:
            ...
            if launch_instance_response.status == 200:
                print("Launch request accepted! Checking instance state...")
                logging_step5.info(
            ...
            if srv_err.code == "LimitExceeded":                
                print(f"LimitExceeded - checking if instance already exists...")
                ...
                if instance_exist_flag:
                    ...
                    sys.exit()
                print("No existing instance found, continuing retries...")     
            data = {
